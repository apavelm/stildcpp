	...
	while ( !p.atEnd() ) {
		...
		while ( i >= 0 ) {
			...
			if ( iconset ) 
			{
				...
				while ( it.hasNext()) 
				{
					SmileIcon *icon = it.next();
					if ( icon->GetRegExp().isEmpty() ) continue;
					...
					do {
						...
						const QRegExp &rx = icon->GetRegExp();
			...
			p.putRich( QString("<icon name=\"%1\" text=\"%2\">").arg(Qt::escape(closest->GetName())).arg(Qt::escape(str.mid(foundPos, foundLen))) );
			i = foundPos + foundLen;
		}
	}
